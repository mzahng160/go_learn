...
func dialTimeout(f newClientFunc, network, address string, opts ...*Option) (client *Client, err error) {
	...
	if err != nil {
		fmt.Println("dialTimeout parseOptions err:", err)
		return nil, err
	}

	conn, err := net.DialTimeout(network, address, opt.ConnectTimeout)
	if err != nil {
		fmt.Println("dialTimeout DialTimeout err:", err)
		return nil, err
	}
	...
	case result := <-ch:
		fmt.Println("dialTimeout finish result ", result.client, " | ", result.err)
		return result.client, result.err
	}
}
...
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	...
	fmt.Println("NewHTTPClient start:", resp, " |err:", err)
